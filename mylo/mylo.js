    ...
    if (topics)
    {
        ...
        for (var topic of topics) 
        {
            ...
            if (forumChannel)
            {
                ...
                let foundYoutube = getYouTubeID(topic.Description.Html);
                if (foundYoutube)
                {
                    message.content += `\nhttps://www.youtube.com/watch?v=${foundYoutube}`;
                }

                
                    let youtubeID = getYouTubeID(html);
                    
                
                    let embed = await getMyLOContentEmbed(root, interaction.guild, youtubeID != null ? "https://www.youtube.com/watch?v="+youtubeID : null);
                    //if there was a youtube url, obtain the thumbnail image for the video id
                    if (youtubeID)
                    {
                        embed.image = { url: `https://img.youtube.com/vi/${youtubeID}/0.jpg` };
                    }
            ...
            else
            {
                ...
                if (foundYoutube)
                {
                    message.content += `\n[YouTube Video](https://www.youtube.com/watch?v=${foundYoutube})`;
                }
...
{ 
    ...
    if (topics)
    {
        for (var topic of topics)
        {
            ...
            {
                message.content = `https://www.youtube.com/watch?v=${foundYoutube}`;
...
export function getYouTubeID(url) {
  ...
  const regex = /(?:v=|embed\/|shorts\/|youtu\.be\/)([a-zA-Z0-9_-]{11})/;
  
  // Try to match the regex against the URL
  const match = url.match(regex);
  
  // If a match is found, return the captured group (the ID), otherwise return null
